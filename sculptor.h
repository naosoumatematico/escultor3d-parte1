...
struct Voxel {
  float r,g,b; // cor do voxel (em sistema aditivo, pois trata de luz); todas devem ser ponto flutuante e variam entre 0 e 1 
...
public:
  ...
  void putVoxel(int x, int y, int z); // ativa o voxel na posicao (x,y,z) e atribui a ele a cor atual do desenho
  void cutVoxel(int x, int y, int z); // desativa o voxel na posicao (x,y,z)
  ...
  void cutEllipsoid(int xcenter, int ycenter, int zcenter, int rx, int ry, int rz); // desativa todos os voxels que satisfazem a equacao do elipsoide
   ...
   // grava o modelo 3D no formato OFF no arquivo filename
